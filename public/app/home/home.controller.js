(function () {
  ...
  function HomeController(AlertMsgService) {
    ...
    function add(task) {
      ...
      AlertMsgService.add({ type: 'success', content: 'Successfully added.' });
      clear();
    }

    function remove(task) {
      vm.tasklist.splice(getIndexOfTask(task), 1);
      AlertMsgService.add({ type: 'success', content: 'Successfully removed.' });
    }
    ...
    function update(task) {
      ...
      vm.clear();
      AlertMsgService.add({ type: 'success', content: 'Successfully edited.' });
    }
